...
if __name__ == "__main__":
    ...
    for k in range(len(biases)):
        ...
        if 'frame_num' not in globals() or frame_num is None:
            ...
            light_angle_increment = 0.05 # 默认角度增量
        elif frame_num > 0:
            light_angle_increment = (2 * math.pi) / frame_num # 每帧旋转的角度，以在frame_num帧内完成一周
        else:
            light_angle_increment = 0.1 # 如果 frame_num 为0或1，使用默认增量
