...
if __name__ == "__main__":
    ...
    select_id__.append(filter_gaussian_points_by_plane(tensor = mpm_init_pos, plane_w=torch.tensor([-1.0, 1.0, 1.0]), plane_b=-0.4, plane_greater=False)[1] )
    ...
    select_id__.append(filter_gaussian_points_by_plane(tensor = mpm_init_pos, plane_w=torch.tensor([1.0, -1.0, 1.0]), plane_b=-0.5, plane_greater=False)[1] )
    mpm_init_pos[:, 2] += 0.3
                        ...
                        if  light_flag : 
                            ...
                            command = f'cd /root/autodl-tmp/debug_physgaussian/cdmpmGaussian/ && source /root/miniconda3/etc/profile.d/conda.sh && conda activate PhysGaussian && python phong_model_wm_shs_15.py \
                                        --npy_path {normal_path} --output_folder {output_folder}  --opacity_path {opacity_path} \
                                            --valid_indice_path {valid_indice_path} --shs_path {shs_path} --color_path {color_path} --light_pos {light_pos} --attenuation_constant {3}'
                            run_command_realtime(command)
