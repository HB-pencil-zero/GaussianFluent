...
if __name__ == "__main__":
    ...
    # mpm_init_pos[:, 1] += 0.7
    # point , size = calculate_minimum_bounding_box_torch (mpm_init_pos)
        
    save_filepath = 'save_bullet_prop.pt'
    
    loaded_data = load_core_init_render_vars(save_filepath, map_location='cuda')
    

    mpm_init_pos = torch.concat([mpm_init_pos, loaded_data["mpm_init_pos"]], dim=0 )
    mpm_init_vol = torch.concat([mpm_init_vol, loaded_data["mpm_init_vol"]], dim=0 )
    mpm_init_cov = torch.concat([mpm_init_cov, loaded_data["mpm_init_cov"]], dim=0 )
    ...
    mpm_solver.import_particle_v_from_torch(torch.zeros(mpm_init_pos.shape[0], 3, device='cuda').add_(torch.tensor([0.0, 0.0, -0.0], device='cuda')))  # -6.0
    # camera setting
    ...
    #     mpm_init_vol=mpm_init_vol,
    #     mpm_init_cov=mpm_init_cov,
    #     opacity_render=opacity_render,
    #     shs_render=shs_render,
    # )
    ...
    azimith_list2 , elevation_list2 = uniform_linear_transition_az_el(start_azimith=45 , target_azimith=-45, start_elevation=0, target_elevation=10, stage_num=40  )

    azimith_list3 , elevation_list3 = azimith_and_elvation_array(start_azimith=-45, azimith_max_delta= 20, start_elevation=10, elevation_max_delta=10, stage_num=10)

    azimith_list4 , elevation_list4 = azimith_and_elvation_array(start_azimith=-45, azimith_max_delta= 20, start_elevation=10, elevation_max_delta=10, stage_num=10)

    azimith_list5, elevation_list5 = uniform_linear_transition_az_el(start_azimith=azimith_list4[-1] , target_azimith=45, start_elevation=elevation_list4[-1], target_elevation=0, stage_num=60  )

    azimith_list6, elevation_list6 =  uniform_linear_transition_az_el(start_azimith=azimith_list5[-1] , target_azimith=45, start_elevation=elevation_list5[-1], target_elevation=0, stage_num=40  )



    azimith_list7, elevation_list7 =  uniform_linear_transition_az_el(start_azimith=azimith_list6[-1] , target_azimith=135, start_elevation=elevation_list6[-1], target_elevation=10, stage_num=60  )
    # azimith_list5 = np.array([ (-45 + 9 *i/2)  for i in range(80)] )

    azimith_list8, elevation_list8 = azimith_and_elvation_array(start_azimith=azimith_list7[-1], azimith_max_delta= 20, start_elevation=elevation_list7[-1], elevation_max_delta=10, stage_num=20)
    
    azimith_list9, elevation_list9 =  uniform_linear_transition_az_el(start_azimith=azimith_list8[-1] , target_azimith=135, start_elevation=elevation_list8[-1], target_elevation=20, stage_num=160  )
    # elevation_list5 = np.array([0  for i in range(80)] )
    azimith_list = np.concatenate([azimith_list1 , azimith_list2 , azimith_list3 , azimith_list4 ,  azimith_list5 , azimith_list6, azimith_list7 , azimith_list8 , azimith_list9 ], axis=0 )
    elevation_list = np.concatenate([elevation_list1 , elevation_list2 , elevation_list3, elevation_list4,   elevation_list5, elevation_list6, elevation_list7 , elevation_list8  , elevation_list9 ], axis=0)

    for frame in tqdm(range(frame_num)):
        # if frame > 120 :
        #     camera_params["delta_e"] = 0
        ...
        if args.render_img:
            ...
            if not args.load_from_saved :
                ...
                loJp = mpm_solver.mpm_state.particle_Jp.numpy() 
            ...
            if color_flag:
                if  light_flag : 
                    ...
                    command = f'cd /root/autodl-tmp/debug_physgaussian/cdmpmGaussian/ && source $(conda info --base)/etc/profile.d/conda.sh && conda activate PhysGaussian && python phong_model_wm_shs_15.py \
                                --npy_path {normal_path} --output_folder {output_folder}  --opacity_path {opacity_path} --valid_indice_path {valid_indice_path} --shs_path {shs_path}'
                    run_command_realtime(command)
            ...
            light_source_position_list = [1.0, 4.5, 1.0]
            light_pos_tensor = torch.tensor(light_source_position_list, dtype=pos.dtype, device=pos.device)

            # 计算 pos_ 中每个点到光源位置的距离
            # distances shape: (N,) where N is the number of points in pos_
            distances = torch.linalg.norm(pos - light_pos_tensor, dim=1)

            # 初始化缩放系数张量
            scales = torch.ones_like(distances) # 默认为1.0

            # 情况1: 距离 < 0.2, 系数固定为 1.1
            scale_tmp = 3
            thresold = 0.7
            mask_close = distances < thresold
            scales[mask_close] = scale_tmp

            # 情况2: 距离 >= 0.2, 系数从1.1递减到1.0
            # 公式: scale = 1.0 + 0.1 * (0.2 / dist)
            # 我们只对 distances >= 0.2 的点应用此规则

            mask_far_or_at_boundary = distances >= thresold
            # 防止除以非常接近0的数（尽管这里有mask_far_or_at_boundary保证了distances[mask_far_or_at_boundary] >= 0.2）
            safe_distances = distances[mask_far_or_at_boundary]
            scales[mask_far_or_at_boundary] = 1.0 + (scale_tmp -1 ) * (thresold / safe_distances )**2
