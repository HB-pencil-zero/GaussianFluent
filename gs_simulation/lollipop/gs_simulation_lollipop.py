...
if __name__ == "__main__":
    ...
    # mpm_init_pos[:, 1] += 1
    _, select_id = filter_tensor_by_hyperplanes_delta(mpm_init_pos, [(torch.tensor([0, 0, 1.0]), 1.32)] , cov=mpm_init_cov, pos=None, cal_bias=0.01, delta=0.21)
    ...
    E[select_id.cpu().numpy()] = 1e5
    mpm_solver.mpm_model.E.assign(E)
    # nu = mpm_solver.mpm_model.nu.numpy()
    # nu[select_id.cpu().numpy()] = 0
    # mpm_solver.mpm_model.nu.assign(nu)
    ...
    # beta[mask__] = 2
    beta[select_id.cpu().numpy()] = 3000000000
    ...
    # substep_dt = cfl * dx / evaluate_sound_speed_linear_elasticity_analysis(1e5, 0.49, rho)
    frame_dt = time_params["frame_dt"]  #/ 4
    ...
    azimuth_list , elvation_list, radius_list, center_list = generate_and_append_ellipse_path(start_azimuth=265, azimuth_max_delta= 20, start_elevation=30, path_radius=7.5, path_center=np.array([    0.41279078, -0.02058649, -1.2324334]), elevation_max_delta=10, stage_num=20)
