...
if __name__ == "__main__":
    ...
    beta[:new_mask.shape[0]][new_mask.cpu().numpy()] = 3000000000
    # beta[mask__] = 2
    ...
    # azimuth_list, elvation_list, radius_list = uniform_linear_transition(265, 350, 30, 10, 7.5, 12.5, 50)

    # 2. 定义包含所有物体的场景配置列表
    #    这里不指定 'scale' 键，函数将自动使用默认值 1.0
    scene_to_create_config = [
        {
            'filepath': 'model/kiwi', 
            'offset': [1.2, 3.5, -0.3742 - 0.0166],
        },
        {
            'filepath': 'model/pineple', 
            'offset': [4.0, 1.0, -0.6465 + 0.3138],
        },
        {
    ...
    # azimuth_list , elvation_list, radius_list, center_list = generate_and_append_ellipse_path(start_azimuth=265, azimuth_max_delta= 20, start_elevation=10, path_radius=7.5, path_center=viewpoint_center_worldspace, elevation_max_delta=10, stage_num=10, \
    ...
    azimuth_list , elvation_list, radius_list, center_list = linear_transition_and_append(
        start_pose=(265, 10, 7.5, viewpoint_center_worldspace),
        target_pose=(310, 10, 12.5, (2, 1, -1.2)),
        stage_num=40,
        existing_azimuths=azimuth_list , existing_elevations=elvation_list, existing_radii=radius_list, existing_centers=center_list )


    azimuth_list , elvation_list, radius_list, center_list = linear_transition_and_append(
        start_pose=(310, 10, 12.5, (2, 1, -1.2)),
        target_pose=(350, 10, 7.5, (1.2150,  3.4929, -0.6749)),
        stage_num=40,
        existing_azimuths=azimuth_list , existing_elevations=elvation_list, existing_radii=radius_list, existing_centers=center_list )
    ...
    for frame in tqdm(range(frame_num)):
        ...
        current_camera = get_camera_view(
            ...
            center_view_world_space=  viewpoint_center_worldspace,  #np.array([2, 1 , -1.2])  ,# np.array([-0.00295043,  0.01138639, -1.0518223 ]), #viewpoint_center_worldspace,
